...
const QuestionPaper = () => {
  ...
  const answerQuestion = async (questionid: any) => {
    if (answer) {
  ...
  const getQuestionBySubjectAndStudent = async (subject: any) => {
    setRefresh(true);
    if (user!.length > 0) {
  ...
  const nextBtn = (questionid: any) => {
    answerQuestion(questionid);
    nextQuestion();
  };

  const submitAnswer = async (subject: any) => {
  ...
  return (
    <React.Fragment>
      <div className="mcontainer">
        ...
        {question
          .slice(indexFirst, lastIndex)
          .map((val: any, idx: any, arr) => {
            return (
